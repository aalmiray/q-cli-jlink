...
2. `cd jreleaser`
3. `rm -rf .gradle && ./gradlew --no-build-cache clean publishToMavenLocal -Prelease=true`


The "nuke cache" options are needed only if you have previously built and published the JReleaser Maven plugins.
